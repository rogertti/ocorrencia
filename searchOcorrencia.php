    ...
    try {
        ...
        $ano = substr($_GET['search_keyword'],6);
            ...
            if(checkdate($mes, $dia, $ano)) {
                ...
                $sql = $pdo->prepare("SELECT idocorrencia,cliente,serial,datado,hora,solicitacao,diagnostico,procedimento,observacao,tecnico,retorno,fechada,desativada,entrega FROM ocorrencia WHERE datado = :datado ORDER BY datado DESC,hora DESC,cliente,serial");
                ...
                $sql = $pdo->prepare("SELECT idocorrencia,cliente,serial,datado,hora,solicitacao,diagnostico,procedimento,observacao,tecnico,retorno,fechada,desativada,entrega FROM ocorrencia WHERE ((serial LIKE :search) OR (cliente LIKE :search) OR (solicitacao LIKE :search) OR (diagnostico LIKE :search) OR (procedimento LIKE :search) OR (observacao LIKE :search)) ORDER BY datado DESC,hora DESC,cliente,serial");
                    ...
                    while($lin = $sql->fetch(PDO::FETCH_OBJ)) {
                        ...
                        <span>
                            ...
                            <p>
                                ...
                                <span><a data-toggle="modal" data-target="#edita-ocorrencia" title="Editar a ocorr&ecirc;ncia" href="editaOcorrencia.php?'.$py.'='.$lin->idocorrencia.'"><i class="fa fa-lg fa-pencil"></i></a></span>
                                '.$ativar.'
                            </p>
                        </span>
                        <hr>';
