<?php
    ...
    $msg = "Campo obrigat&oacute;rio vazio.";
        ...
        if(empty($_POST['stamp'])) { die("reload"); }
        ...
        if(empty($_POST['viacliente'])) { $_POST['viacliente'] = 'F'; }
        ...
        if($filtro == 5) {
            try {
                ...
                $sql = $pdo->prepare("SELECT idocorrencia FROM ocorrencia WHERE serial = :serial");
                ...
                $ret = $sql->rowCount();
                    ...
                    if($ret > 0) {
                        ...
                        $fechada = 'F';
                        $desativada = 'F';
                        $sql2 = $pdo->prepare("SELECT idocorrencia FROM ocorrencia WHERE datado = :datado AND hora = :hora AND tecnico = :tecnico AND fechada = :fechada AND desativada = :desativada");
                ...
                $res = $sql->execute();
                    ...
                    if(!$res) {
                        ...
                        if(($_POST['fechada'] == "T") or ($_POST['entrega'] == "T")) {
                            ...
                            echo'<url>valorOcorrencia.php?'.$pyocorrencia.'='.$idocorrencia.'&'.$pyserial.'='.$_POST['serial'].'</url>';
                            unset($idocorrencia,$pyocorrencia,$pyserial);
                        } elseif($_POST['viacliente'] == "T") {
                            $idocorrencia = $pdo->lastInsertId();
                            $pyocorrencia = md5('idocorrencia');
                            echo'<url>printViaOcorrencia.php?'.$pyocorrencia.'='.$idocorrencia.'</url>';
                            unset($idocorrencia,$pyocorrencia);
                        } else {
                            echo'true';
                        }
                    }
